...
if (chats.startsWith("@6283168004413")){
		...
		switch(command) {
			...
			case prefix+'help':
			  conn.sendMessage(from, { audio: fs.readFileSync('audio/Assalamualaika.m4a'), mimetype: 'audio/mp4', ptt: true}, {quoted: msg})
				...
				/*conn.sendMessage(from, { react: { text: `👋`, key: msg.key }})*/
conn.sendMessage(from, { caption: teks, image: fs.readFileSync('media/Jojo2.jpg'), templateButtons: buttonsDefault, footer: 'Farbot', mentions: [sender]} )
				break
case prefix+'sewa':
  case prefix+'daftarprem':
  var teks = `*[ SEWA BOT ]*

jika ingin sewa bot harap hubungi owner ya kak
5K PERMANEN`
			    conn.sendMessage(from, { caption: teks, location: { jpegThumbnail: fs.readFileSync(setting.pathimg) }, templateButtons: button5, footer: 'Farbot', mentions: [sender] })
			    break
			case prefix+'runtime':
			...
			case prefix+'owner':
			    for (let x of ownerNumber) {
			      tact(from, x.split('@s.whatsapp.net')[0], 'Owner', msg)
			    }
			    break
			case prefix+'cekprem':
...
case prefix+'meme':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
				reply(mess.wait)
			    var query = ["meme","meme dark","darkjokes","meme","meme lucu"]
                var data = await pinterest(pickRandom(query))
				var but = [{buttonId: `/loli`, buttonText: { displayText: "Meme lagi" }, type: 1 }]
				conn.sendMessage(from, { caption: "Random meme", image: { url: pickRandom(data.result) }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: msg })
			    limitAdd(sender, limit)
 			    break
case prefix+'waifu':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
...
default:
			if (!isGroup && isCmd) {
				reply(`Command ${command} Belum Tersedia Di Dalam ${prefix}menu`)
			}
		}
	} catch (err) {
